...
{		
	...
	for (auto itAlbum = m_albums.begin(); itAlbum != m_albums.end(); ++itAlbum)
	{
		// Check if it's a users album 
		if (userId.getId() == itAlbum->getOwnerId())
		{
			// Delete users album
			itAlbum = m_albums.erase(itAlbum);
		}
		else
		{
			itAlbum->untagUserInAlbum(userId.getId());
		}
	}
