...
void AlbumManager::copyPicture()
{
	...
	std::string pictureName = getInputFromConsole("Enter picture name: ");
	if (!m_openAlbum.doesPictureExists(pictureName)) 
	{
		throw MyException("Error: There is no picture with name <" + pictureName + ">.\n");
	}

	Picture picture = m_openAlbum.getPicture(pictureName);

	std::string duplicateFilePath = WinAPIFunc::copyPicture(picture);
	if (duplicateFilePath == "NULL")
	{
		std::cout << "Picture with id: " << picture.getId() << ", was not copied." << std::endl;
		return;
	}

	Picture duplicatePicture(++m_nextPictureId, "CopyOf_" + picture.getName());
	duplicatePicture.setPath(duplicateFilePath);

	m_dataAccess.addPictureToAlbumByName(m_openAlbum.getName(), duplicatePicture);
	std::cout << "Picture with id: " << picture.getId() << ", copied successfully." << std::endl;
}
...
void AlbumManager::removeUser()
{
	...
	{
		int userId = std::stoi(userIdStr);
		if (!m_dataAccess.doesUserExists(userId)) { // If user doesn't exist 
			throw MyException("Error: There is no user with id @" + userIdStr + "\n");
		}
		const User& user = m_dataAccess.getUser(userId);
		if (isCurrentAlbumSet() && userId == m_openAlbum.getOwnerId()) { // Close all windows of the user to delete
			closeAlbum();
		}
...
void AlbumManager::userStatistics()
{
	...
	std::cout << "user @" << userId << " Statistics:" << std::endl << "--------------------" << std::endl <<
		...
		"  + Number of owned albums: " << m_dataAccess.countAlbumsOwnedOfUser(user) << std::endl;
}
...
void AlbumManager::openPicture()
{
	...
	Picture pic = m_openAlbum.getPicture(pictureName);
	...
	{
		std::cout << "This program isn't supported." << std::endl;
	}
}
